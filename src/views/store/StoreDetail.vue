...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    methods: {
      addOrRemoveShelf () {
        if (this.inBookShelf) {
          this.setShelfList(removeFromBookShelf(this.bookItem)).then(() => { saveBookShelf(this.shelfList) })
        } else {
      ...
      readBook () {
        this.$router.push({
          path: `/ebook/${this.bookItem.categoryText}|${this.fileName}`// this.categoryText
        })
      },
      trialListening () {
        getLocalForage(this.bookItem.fileName, (err, blob) => {
          if (!err && blob && blob instanceof Blob) { // 离线朗读
            this.$router.push({
              path: '/store/speaking',
              query: {
      ...
      read (item) {
        ...
        // })
        getLocalForage(this.fileName, (err, value) => {
    ...
    mounted () {
      this.init()
      if (!this.shelfList || this.shelfList.length === 0) this.getShelfList()// 增加判断减少开销
    }
  }
</script>
